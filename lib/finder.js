...
function findOrphans(context) {
  const { logger, findModules, config } = context;
  assert(config.rootPath, "rootPath config option not set");
  return Promise.all([
    findAllLinks(context),
    readdir(config.rootPath, {
      filter: stats =>
  ...
  ]).then(([knownLinks, foundLinks]) => {
    logger.debug("Expecting links ", knownLinks);
    const orphans = foundLinks.reduce((orphans, [source, dest]) => {
      if (
        knownLinks[source.name] &&
        knownLinks[source.name].includes(dest.name)
      ) {
        return orphans;
      }
      return Object.assign({}, orphans, {
        [source.name]: [...(orphans[source.name] || []), dest.name]
      });
    }, {});
    logger.debug("Found links", orphans);
    return orphans;
  });
}
